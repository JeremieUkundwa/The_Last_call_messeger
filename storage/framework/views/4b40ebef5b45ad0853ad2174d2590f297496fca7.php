<?php $__env->startSection('donate-section'); ?>
<main>
        <div id="main-body" style="display: flex;
  min-height: 50vh;
  flex-direction: column;
  justify-content: space-between;">
          <h1>Dufashe kwamamaza inkuru ya Yesu Kristo utanga inkunda yawe</h1> <br>
          <p>
          “Nababereye icyitegererezo muri byose, yuko ari ko namwe mukwiriye gukora imirimo ngo mubone uko mufasha abadakomeye, no kwibuka amagambo Umwami Yesu yavuze ati ‘Gutanga guhesha umugisha kuruta guhabwa.” Ibyakozwe n’Intumwaa 20:35
          </p> <br>
          <div class="container">
            <div class="row" style="width: 65vw;">
              <div class="col-sm">
                <img src="./images/mtn-logo.png" alt="">
                <p class="inner-par">+250782799851</p>
              </div>
              <div class="col-sm">
                <img src="./images/logo.wu.big.svg" style="background-color: rgb(7, 7, 99);" alt="">
                <p class="inner-par"></p>
              </div>
              <div class="col-sm"><img src="./images/World-remit-logo.jpeg" alt=""></div>
              <div class="col-sm">
                <img src="./images/kcb.png" alt="">
                <p class="inner-par">4401316835</p>
              </div>
          </div>
      </main>
      
<?php $__env->stopSection(); ?>
<?php echo $__env->make('layout', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/jeremie/Desktop/project/laravel/the_last_call_messeges/resources/views/donate.blade.php ENDPATH**/ ?>